	...
	char *fmt, ...)
{
   ...
   /* format the message: */
	...
	if (!(fullid = glite_lbu_JobIdUnparse(ctx->p_jobid))) { 
		edg_wll_SetError(ctx,ret = EINVAL,"edg_wll_FormatLogLine(): glite_lbu_JobIdUnparse() error"); 
		goto edg_wll_formatlogline_end;
	}
        ...
        struct timeval *timeout)
{
	...
	if (!(fullid = glite_lbu_JobIdUnparse(ctx->p_jobid))) { 
		ret = edg_wll_SetError(ctx,EINVAL,"edg_wll_LogFlush(): glite_lbu_JobIdUnparse() error");
		goto edg_wll_logflush_end;
	}
	...
	int flags)
{
	...
	glite_lbu_JobIdFree(ctx->p_jobid);
	if ((err = glite_lbu_JobIdDup(job,&ctx->p_jobid))) {
		edg_wll_SetError(ctx,err,"edg_wll_SetLoggingJob(): glite_lbu_JobIdDup() error");
        ...
        int flags)
{
	...
	char	*code_loc = NULL;
        ...
        glite_lbu_JobIdFree(ctx->p_jobid);
        if ((err = glite_lbu_JobIdDup(job,&ctx->p_jobid))) {
                edg_wll_SetError(ctx,err,"edg_wll_SetLoggingJobProxy(): glite_lbu_JobIdDup() error");
